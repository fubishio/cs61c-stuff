...
NEXTLINE:
    ...
    while(fgets(buf, BUF_SIZE, input)) {
        input_line++;
        *line = buf;
        if (buf == NULL || strlen(buf) == 0) continue;
        ...
        int check = add_if_label(input_line, token, byte_offset, symtbl);

        // Scan for arguments
        char* instruction_name;
        int num_args = 0;
        if (check == 0) instruction_name = token;
        if (check == 1) instruction_name = strtok(NULL, IGNORE_CHARS);
        if (check == -1) {success = -1; /*continue;*/instruction_name = strtok(NULL, IGNORE_CHARS);}
        if (instruction_name == NULL) goto NEXTLINE;
        
        // CREATES SEG FAULT, cant use :(
        // int yn = 0;
        // for (int i = 0; i < strlen(token); i++) {
        //     if (yn == 1) continue;
        //     //would put i < 7 but afraid of some fun errors
        //     if (strcmp((char) token[i], "%s")) {
        //         token[7] = '\0';
        //         yn = 1;
        //     }
        // }
        ...
        // for(int j = 0; j<3; j++) {
        //     args[j] = (char*) malloc(sizeof(char)*BUF_SIZE);
        // }
        // int num_args = malloc(sizeof(int));
        char *temp;
        for(int i = 0; i < MAX_ARGS; i++) {
            args[i] = strtok(NULL, IGNORE_CHARS);
            // if (temp == NULL) {
            //     i = MAX_ARGS;
            // }
            // strcpy(args[i], temp);
        ...
        // if (temp != NULL) {raise_extra_arg_error(input_line, temp); success = -1; goto NEXTLINE;}
    	// Checks to see if there were any errors when writing instructions
        unsigned int lines_written = write_pass_one(output, instruction_name, args, num_args);

        if (lines_written == 0) {
            //tried to use trans_int but didnt pass errors
            if (num_args == 0) {fprintf(output, "%s\n", instruction_name);}
            else {raise_inst_error(input_line, token, args, num_args); success = -1;}
        }
        // for(int k = 0; k<3; k++) {
        //     free(args[k]);
        // }
...
int pass_two(FILE *input, FILE* output, SymbolTable* symtbl, SymbolTable* reltbl) {
    ...
    while(fgets(buf, BUF_SIZE, input)) {
     ...
     char temp[50];
     strncpy(temp, strtok(buf, IGNORE_CHARS), 10);
     //temp[11] = NULL; 
     temp[11] = '\0';
     
     char* cond = strtok(NULL, IGNORE_CHARS);
     char* args[MAX_ARGS];
     int num_args = 0;
     while  (cond != NULL) {
        args[num_args] = strtok(NULL, IGNORE_CHARS);
        num_args++;

     }

     int check = translate_inst(output, temp, args, num_args, byte_offset*4, symtbl, reltbl);
     if (check == -1){
        raise_inst_error(byte_offset, temp, args, num_args); 
        success = -1;
    }
     byte_offset++;
