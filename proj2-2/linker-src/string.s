...
loop:
	lb $t1, 0($t0)
	beq $t1, $zero, return
	addiu $v0, $v0, 1 		#increment return value
	addiu $t0, $t0, 1 		#iterates
	j loop
return:
	jr $ra
...
#------------------------------------------------------------------------------
strncpy:
	move $t0, $a0		#place to copy
	move $t1, $a1		#being copied
	move $t2, $a2		#counter
loop1:
	beq $t2, $zero, return1
	lbu $t3, 0($t1)		#store temporary for copying
	sb $t3, 0($t0)
	addiu $t0, $t0, 1 		#incrementing
	addiu $t1, $t1, 1
	subiu $t2, $t2, 1 		#lowering counter
	j loop1
return1:
	move $v0, $a0
	jr $ra
