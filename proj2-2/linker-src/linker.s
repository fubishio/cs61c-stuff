...
write_machine_code:
	...
	move $s5, $a0			# $s5 = output file ptr
	move $s4, $a1			# $s4 = input file ptr
	move $s3, $a2			# $s3 = symbol table
	move $s2, $a3			# $s2 = relocation table
...
write_machine_code_next_inst:
	...
	jal relocate_inst
	li $t4, -1
	beq $v0, $t4, write_machine_code_error
	move $s0, $v0
	
	
write_machine_code_to_file:
	# 6. Write the instruction into a string buffer via hex_to_str():
	la $a1, hex_buffer #can't find where hex_buffer is given, but its already given 
	move $a0, $s0
	jal hex_to_str
