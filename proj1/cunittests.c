 ...
 * Checks the case where someone tries to remove the same
 * file twice (which should error) and also checks other 
 * basic functionality.
 */
void remove_test(void)
{
    ...
    FILE* asdf = fopen("asdf.txt", "w");
    fclose(asdf);
    FILE* qwer = fopen("qwer.txt", "w");
    fclose(qwer);
    retval = beargit_add("asdf.txt");
    CU_ASSERT(0==retval);
    retval = beargit_add("qwer.txt");
    CU_ASSERT(0==retval);

    // Remove both files at different times
    run_commit(&commit_list, "THIS IS BEAR TERRITORY!1");
 ...
 * Checks the that basic functionality of reset works.
 */
void reset_test(void)
{
    ...
    CU_ASSERT(0==retval);
    run_commit(&commit_list, "THIS IS BEAR TERRITORY!2");
    char commit_id2[COMMIT_ID_SIZE];
    read_string_from_file(".beargit/.prev", commit_id2, COMMIT_ID_SIZE);
    
    // Run reset on a nonexistent commit id
    retval = beargit_reset("blah", "qwer.txt");
    CU_ASSERT(1==retval);

    // Run reset on a valid file and id
    retval = beargit_reset(commit_id, "asdf.txt");
    CU_ASSERT(0==retval);

    // Modify an existing file and run reset on it
    write_string_to_file("qwer.txt", "blah");
    retval = beargit_reset(commit_id, "qwer.txt");
    CU_ASSERT(0==retval);

    // Verify the file has returned to its original state
    // Verify that the removed file is in the directory
    char* str;
    read_string_from_file("qwer.txt", str, 0);
    CU_ASSERT(str=="");
    free_commit_list(&commit_list); 
}
...
int cunittester()
{
   ...
   if (NULL == CU_add_test(pSuite3, "Remove Test", remove_test))
   {
      CU_cleanup_registry();
      return CU_get_error();
   }
   ...
   if (NULL == CU_add_test(pSuite4, "Reset Test", remove_test))
   {
      CU_cleanup_registry();
      return CU_get_error();
   }
